...
//   </svg>
// );
// import { useClerk, useUser, UserButton, SignInButton } from "@clerk/react";
// const Nav = () => {
//   const navLinks = [
//     { name: "Home", path: "/" },
...
const Nav = () => {
  ...
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const { openSignIn } = useClerk();
  const { user, nav, isOwner, sshg } = useAppContext();
  const lc = useLocation();
  console.log(isOwner);
  ...
  useEffect(() => {
    if (lc.pathname !== "/") {
      setIsScrolled(true);
      return; // no scroll listener for other pages
    }
    ...
    >
      ...
      <Link to="/">
        <img
          src={assets.logo}
          alt="Logo"
          className={`${isScrolled ? "invert opacity-80" : ""} h-9`}
        />
      </Link>
      ...
      <div className="hidden md:flex items-center gap-4 lg:gap-8">
        ...
        {user && (
          <button
            onClick={() => (isOwner ? nav("/owner") : sshg(true))}
            className={`border px-4 py-1 text-sm font-light rounded-full cursor-pointer transition-all ${
